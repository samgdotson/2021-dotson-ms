...
time_horizon = np.arange(2025, 2051, 5)
...
elc_techs = ['NUCLEAR_EXISTING',
             ...
             # 'SOLAR_RESIDENTIAL',
             'NUCLEAR_TB']
ind_techs = ['NUCLEAR_NEW', 'ABBOTT']
chw_techs = ['CWS', 'TES']
vcl_techs = ['GSLVCL', 'ELCVCL', 'H2VCL']
emissions = {'CO2eq': [
                       'NUCLEAR_NEW',
                       'SOLAR_FARM',
                       # 'SOLAR_RESIDENTIAL',
...
def create_dataframe(
    ...
    if save:
        if emission is not None:
            dataframe.to_csv(f'{name}_{sector}_{variable}_{emission}.csv')
        else:
    ...
    for file in data_paths:
        ...
        for var in variables:
            ...
            df_ind = create_dataframe(datalines,
                                      ...
                                      name=scenario,
                                      sector='ind')
            df_vcl = create_dataframe(datalines,
                                      var,
                                      name=scenario,
                                      sector='vcl')
            df_chw = create_dataframe(datalines,
                                      var,
                                      name=scenario,
                                      sector='chw')
            dfs.append(df_elc)
